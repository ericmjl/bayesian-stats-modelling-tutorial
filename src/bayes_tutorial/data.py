...
def load_finches_2012():
    path = here() / "data/finch_beaks_2012.csv"
    return load_finches(path)


def load_finches_1975():
    path = here() / "data/finch_beaks_1975.csv"
    df = (
        load_finches(path)
...
def load_finches(path):
    ...
    df = (
        ...
        .rename_column(
            "blength", "beak_length"
        )  # rename blength to beak_length (readability fix)
        .rename_column(
            "bdepth", "beak_depth"
        )  # rename bdepth to beak_depth (readability fix)
        .label_encode(
            "species"
        )  # create a `species_enc` column that has the species encoded numerically
    )
    return df
...
def load_kruschke():
    df = pd.read_csv(
        here() / "data/iq.csv", index_col=0
    ).label_encode(  # comment out the path to the file for students.
        "treatment"
    )
    return df
...
def load_decay():
    ...
    def decay_func(ts, noise):
        return (
            A * np.exp(-t / tau)
            + C
            + np.random.normal(0, noise, size=(len(t)))
        )
...
def load_ice_cream():
    data = (
        pd.read_csv(here() / "data/ice_cream_shop.csv", index_col=0)
        .reset_index(drop=True)
        .select_columns(["shopname", "num_customers", "owner_idx", "num_favs"])
    )
    return data
