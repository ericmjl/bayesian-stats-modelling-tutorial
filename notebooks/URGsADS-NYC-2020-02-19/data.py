...
def load_finches_1975():
    ...
    df = df.rename_column("beak_length_mm", "beak_length").rename_column(
        "beak_depth_mm", "beak_depth"
    )
    return df
...
def load_sterilization():
    df = (
        pd.read_csv(
            "../data/sterilization.csv", na_filter=True, na_values=["#DIV/0!"]
        )
        .clean_names()
